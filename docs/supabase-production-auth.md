...
`PRELUDE_MATCH_RECIPIENT` and `PRELUDE_FROM_EMAIL` power `POST /api/prelude-match/submit` (Cloudflare Pages Function). If unset, the recipient defaults to `prelude@preludeconsultingllc.com` and the sender falls back to `AUTH_EMAIL_FROM`. The sending address must be verified in Resend.
