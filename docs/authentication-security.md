...
For local development, the simplest option is to copy `.env.example` to `.env`; Vite loads `.env` automatically and the auth middleware now copies those values into `process.env` for Prisma. If you prefer shell variables on Windows, use Command Prompt syntax such as `set DATABASE_URL=postgresql://...` or PowerShell syntax such as `$env:DATABASE_URL="postgresql://..."`; Unix-style `DATABASE_URL="..."` assignments do not work in `cmd.exe`.

If Vite moves to another port, either leave `PUBLIC_APP_URL` unset so links are derived from the current request host, or update it to the actual URL shown by Vite, including `/Preludev1`.
