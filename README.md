...
Billing can remain disabled for local development. To enable Stripe, configure the Stripe secret key, webhook secret, and plan price IDs expected by the billing config. The SAT & ACT Prep and Academic Tutoring price IDs are generated by the catalog script for service products.
...
STRIPE_PRICE_ID_SAT_ACT_PREP=price_...
STRIPE_PRICE_ID_ACADEMIC_TUTORING=price_...
```
...
Run `node scripts/setup-stripe-catalog.mjs` to create the Basic, Plus, Pro, SAT & ACT Prep, and Academic Tutoring Stripe products and monthly prices. Add `--write-env` to write the generated price IDs into `.env`.
