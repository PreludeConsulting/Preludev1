...
function makeAppUrl(req, path) {
  return `${getAppBaseUrl(req)}${path.startsWith("/") ? path : `/${path}`}`;
}
...
async function handleRegister(req, res) {
  ...
  const verifyUrl = makeAppUrl(req, `/verify-email?token=${result.verificationToken}`);
...
async function handleRequestReset(req, res) {
  ...
  if (user) {
    ...
    sendDevEmail("password-reset", user.email, makeAppUrl(req, `/reset-password?token=${token}`));
  }
...
async function handleMe(req, res) {
  const auth = await getAuth(req);
  if (!auth) return sendJson(res, 401, { error: "unauthenticated", message: "Authentication required." });
...
export function createAuthApiMiddleware() {
  return async function authApiMiddleware(req, res, next) {
    ...
    try {
      ...
      sendJson(res, statusCode, { error: statusCode === 401 ? "unauthenticated" : statusCode === 403 ? "forbidden" : statusCode >= 500 ? "server_error" : "request_failed", message: error.message || "Request failed." });
    }
  };
}
